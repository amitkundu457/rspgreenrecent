...
const Index = ({ workOrders, user, notif, user_type }) => {
    ...
    const [workOrderss, setWorkOrders] = useState(false);
    ...
    const updateWorkOrder = async () => {
        try {
            ...
            setWorkOrders(
                workOrders.map((wo) => (wo.id === data.id ? response.data : wo))
            );
            resetForm();
        } catch (error) {
                        ...
                        {workOrders.map((workOrder) => (
                            <tr key={workOrder.id} className="hover:bg-gray-50">
                                <td className="border px-4 py-2">
                                    {workOrder.seller_name}
                                </td>
                                <td className="border px-4 py-2">
                                    {workOrder.seller_address}
                                </td>
                                <td className="border px-4 py-2">
                                    {workOrder.date_of_wo}
                                </td>
                                <td className="border px-4 py-2">
                                    {workOrder.subject}
                                </td>
                                <td className="border px-4 py-2">
                                    {workOrder.contact_person_name}
                                </td>
                                <td className="border px-4 py-2 flex items-center gap-2">
                                    ...
                                    <Link
                                        href={`/seller-workOrder/${workOrder.id}/show`}
