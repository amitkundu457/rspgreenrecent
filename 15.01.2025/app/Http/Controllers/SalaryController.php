...
class SalaryController extends Controller
{
    ...
    {
        // Fetch combined data for the employee
        $combinedData = DB::table('branches')
            ->join('holidays', 'branches.id', '=', 'holidays.id')
            ->join('employees', 'employees.branch_id', '=', 'branches.id')
            ->join('locations_holiday', 'branches.location_id', '=', 'locations_holiday.id')
        ...
        $combinedData = $combinedData->map(function ($item) {
            ...
            // Adjusted leave days considering holidays
            $adjustedLeaveDays = $totalLeaveDays - $holidayInLeavePeriod;
        ...
        // Update salary records based on deductions and allowances
        $salaries = Salary::whereIn('employee_id', $combinedData->pluck('employee_id'))->get();
        ...
        foreach ($salaries as $sal) {
            ...
            // Fetch employee data for deductions
            $employeeData = $combinedData->firstWhere('employee_id', $sal->employee_id);
        ...
        // Render the salary PDF view with combined data
